...
tickers = map_dfr(.x = "ADR", .f = methodsPPI::sets, server = server, port = port) %>% select(ticker) %>% pull(ticker)
nombres = tickers
tickers = c(tickers, "^GSPC","^NDX")
nombres = c(nombres, "SPX", "NDX")
nombres_display = data.frame("ticker" = tickers, "nombre" =nombres)

orden <- c("BANCOS", "GENERADORAS", "UTILITIES", "PETROLEO", "BASIC MATERIALS", "REAL ESTATE", "TELCO")
prices_raw <- tidyquant::tq_get(tickers, from = Sys.Date() - lubridate::years(2), to = Sys.Date() + 1)
datos <- prices_raw %>%
  dplyr::select(date, ticker = symbol, price = adjusted) %>%
  tidyr::drop_na() %>%
  dplyr::arrange(ticker, date)
mrv = getMerval(fechaInicio = Sys.Date() - lubridate::years(2), server = server, port = port) %>% 
  select(date, price = mervalCCL) %>% mutate(ticker = "Merval CCL") %>% relocate(date, ticker, price)
mrv = mrv %>% distinct(date, .keep_all = TRUE) # el 2026-03-13 yahoo devolvía doble el valor
tickers = c(tickers, "Merval CCL")
full = rbind(datos, mrv) %>% arrange(ticker, date)
