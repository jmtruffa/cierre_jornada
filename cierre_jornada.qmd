...
```{r mulc, echo = FALSE}
knitr::opts_chunk$set(comment = "")
if (!exists("mulc", inherits = TRUE)) {

  mulc_path <- file.path(path, "mulc.rds")

  if (!file.exists(mulc_path)) {
    warning(sprintf(
      "No existe %s. Ejecutá cierre_mulc.R para generarlo.",
      mulc_path
    ))
    mulc <- NULL
  } else {
    mulc <- readRDS(mulc_path)
  }
}

# Uso del objeto si existe
if (!is.null(mulc)) {
  mulc %>%
    tail() %>%
    flextable() %>%
    width(j = "date", width = 1) %>%
...
knitr::opts_chunk$set(comment = "")
if (!exists("reservas", inherits = TRUE)) {

  reservas_path <- file.path(path, "reservas.rds")

  if (!file.exists(reservas_path)) {
    warning(sprintf(
      "No existe %s. Ejecutá cierre_reservas.R para generarlo.",
      reservas_path
    ))
    reservas <- NULL
  } else {
    reservas <- readRDS(reservas_path)
  }
}

# A partir de acá se usa `reservas` si existe
if (!is.null(reservas)) {
  reservas %>%
    mutate(var = valor - lag(valor)) %>%
    tail() %>%
    select(date, reservasTXT, var) %>%
    flextable() %>%
    width(j = "date", width = 1) %>%
    align(align = "center", part = "all")
}
...
if (!exists("tabla_fx", inherits = TRUE)) {

  tabla_fx_path <- file.path(path, "tabla_fx.rds")

  if (!file.exists(tabla_fx_path)) {
    warning(sprintf(
      "No existe %s. Ejecutá el script correspondiente para generarlo.",
      tabla_fx_path
    ))
    tabla_fx <- NULL
  } else {
    tabla_fx <- readRDS(tabla_fx_path)
  }
}
...
if (exists("depos", inherits = TRUE) && exists("tc", inherits = TRUE)) {
  depos %>% 
    left_join(tc) %>% 
...
```{r merval, echo = FALSE}
if (exists("merval", inherits = TRUE)) {
